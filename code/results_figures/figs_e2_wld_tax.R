...
# read in data 
data <- read_csv(file.path(path,'welfare_carbontaxWLD_l_50.csv'))
...
p1 <- ggplot() +
  ...
  geom_sf(data = world_data, aes(fill = dW), color = "grey90", size = 0.1) +
  ...
  labs(
    ...
    subtitle = "from a $50/ton global carbon tax"
  ) +
  theme_minimal() +
  theme(
...
savename <- 'welfare_carbontaxWLD_50usd_map.png'
...
savename <- 'welfare_carbontaxWLD_50usd_bar.png'
...
p1 <- ggplot() +
  geom_sf(data = sf::st_graticule(ndiscr = 10000, margin = 0.05), 
          color = "grey70", size = 0.05, alpha = 0.5) +
  geom_sf(data = world_data, aes(fill = (dW_p)/dW), color = "grey90", size = 0.1) +
  scale_fill_gradient2(
    low = "firebrick3", 
    high = "dodgerblue3",
    midpoint=0,
    name = "Share of welfare",
    na.value = "grey85"
  ) +
  coord_sf(crs = st_crs("+proj=robin"), # Robinson projection
           datum = NA) +
  theme_minimal() +
  labs(
    title = "Share of welfare attributed to trade effects",
    subtitle = "from a $50/ton global carbon tax"
  ) +
  theme_minimal() +
  theme(
...
p1 <- ggplot() +
  geom_sf(data = sf::st_graticule(ndiscr = 10000, margin = 0.05), 
          color = "grey70", size = 0.05, alpha = 0.5) +
  geom_sf(data = world_data, aes(fill = (dW_e+dW_π)/dW), color = "grey90", size = 0.1) +
  scale_fill_gradient2(
    low = "firebrick3", 
    high = "dodgerblue3",
    midpoint=0,
  ...
  theme_minimal() +
  labs(
    title = "Share of welfare attributed to energy market effects",
    subtitle = "from a $50/ton global carbon tax"
  ) +
  theme_minimal() +
  theme(
