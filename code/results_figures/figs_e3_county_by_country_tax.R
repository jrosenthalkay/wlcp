...
# read in data 
data <- read_csv(file.path(path,'welfare_renewsubsidy_unilateral_i_l_50.csv'))
...
p1 <- ggplot() +
  ...
  labs(
    ...
    subtitle = "from a unilateral 42.6% renewable energy subsidy"
  ) +
  theme_minimal() +
  theme(
...
savename <- 'welfare_renewsubsidy_unilateral_i_50_map.png'
ggsave(file.path(savepath,savename), plot = p1, width = 8, height = 6, dpi = 300)

# now world #

# read in data 
data2 <- read_csv(file.path(path,'welfare_renewsubsidy_unilateral_w_l_50.csv'))
...
p2 <- ggplot() +
  ...
  theme_minimal() +
  labs(
    title = "The global welfare gain from unilateral renewable energy subsidies",
    subtitle = "from a unilateral 42.6% renewable energy subsidy"
  ) +
  theme_minimal() +
  theme(
...
savename <- 'welfare_renewsubsidy_unilateral_w_50_map.png'
