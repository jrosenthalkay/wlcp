...
# read in data 
data <- read_csv(file.path(path,'welfare_carbontax_carbontariff_As_l_50.csv'))
...
p1 <- ggplot() +
  ...
  labs(
    ...
    subtitle = "from an ASEAN-wide CBAM and carbon tax of $50"
  ) +
  theme_minimal() +
  theme(
...
savename <- 'welfare_ASEAN_CBAM_map.png'
...
savename <- 'welfare_ASEAN_CBAM_bar1.png'
...
# Filter for the specified countries
selected_countries1 <- c("BRN", "KHM", "IDN", "LAO", "MYS", "MMR", "PHL", "SGP", "THA", "VNM")
selected_countries2 <- c("USA", "CHN", "GBR", "RUS", "TUR", "IND", "BRA", "WLD", "NGA", "DEU", "FRA", "SAU")
filtered_data <- data %>%
...
savename <- 'welfare_ASEAN_CBAM_bar2.png'
